...
import { Field, Modal, Status } from '../components/ui.jsx';

const emptyNumber = () => ({ inboundNumber: '', callerId: '', destinationType: 'unassigned', destinationId: '' });
const newDraft = () => ({ id: crypto.randomUUID(), revision: 0, name: '', provider: '', accountReference: '', server: '', port: 5060, transport: 'UDP', publicIp: '', hostingProvider: '', authentication: 'unconfirmed', username: '', numbers: [], notes: '' });
...
export function CarrierTrunksPanel({ api, config, extensions }) {
  ...
  return <section className="band">
    <div className="section-title"><div><h2>Carrier trunks</h2><p>Company-owned carrier details and per-number routing configuration.</p></div><button className="secondary" disabled={loading} onClick={() => { setError(''); setDraft(newDraft()); }}><Plus /> Add carrier trunk</button></div>
    {error && <div className="error-banner" role="alert">{error}</div>}
    {loading ? <p role="status">Loading carrier trunks…</p> : trunks.length ? <div className="object-list">{trunks.map(item => <div key={item.id}>
      <span className="object-icon"><Server /></span><div><strong>{item.name}</strong><small>{item.provider} · {item.server}:{item.port} / {item.transport} · {item.numbers.length} numbers</small><small>Public IP {item.publicIp} · {item.hostingProvider || 'Hosting not specified'}</small></div>
      <Status warn>Pending activation</Status><div className="row-actions"><button onClick={() => { setError(''); setDraft(structuredClone(item)); }}>Edit configuration</button></div>
    </div>)}</div> : <p>No carrier configurations have been added for this company.</p>}
    ...
    {draft && <Modal wide title={draft.revision ? `Edit ${draft.name}` : 'Add carrier trunk'} subtitle={config.company.name} onClose={() => { if (!busy) setDraft(null); }}><form className="modal-form form-grid" onSubmit={save}>
      ...
      <div className="wide"><div className="section-title"><div><h3>Inbound numbers and destinations</h3><p>Keep destinations unassigned until you are ready to configure them.</p></div><button type="button" className="secondary" onClick={() => field('numbers', [...draft.numbers, emptyNumber()])}>Add number</button></div>
      {draft.numbers.map((number, index) => <div className="form-grid" key={index} style={{ marginBottom: 16 }}>
        ...
        <button type="button" className="secondary" onClick={() => field('numbers', draft.numbers.filter((_, i) => i !== index))}>Remove number {index + 1}</button>
      </div>)}</div>
