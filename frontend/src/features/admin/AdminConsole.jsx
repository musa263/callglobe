...
import { Building2, ChevronDown, KeyRound, PhoneCall, RefreshCw, Save, ShieldCheck, X } from "lucide-react";
...
import { Toggle, Field, Modal } from "./components/ui.jsx";
...
export default function AdminConsole({ profile }) {
  ...
  const api = workspaceApi(request, organizationId, () => workspaceRef.current === organizationId && renderGeneration === loadGeneration.current);
  async function load(selectedId = organizationId) {
    ...
    try {
      ...
      const [u, w, o, t, n, v, k, b, e] = await Promise.all([
        ...
        allowed('aiReceptionist') ? api('/api/voice/settings') : null,
      ...
      setBusiness(b?.config || { enabled: false, companyName: p.config.company.name, greeting: '', waitingMessage: '', departments: [], voice: '', backgroundImageUrl: '' });
      return true;
    } catch (err) {
  ...
  const saveUser = async (event) => { event.preventDefault(); setBusy(true); setError(''); try { const editing = Boolean(userDraft.id); const result = await api('/api/admin/extensions', { method: editing ? 'PATCH' : 'POST', body: userDraft }); const saved = result.extension; setUserDraft({ ...userDraft, id: saved.id }); const fresh = await api('/api/admin/pbx'); const p = await api('/api/admin/pbx', { method: 'PUT', body: { ...fresh.config, userProfiles: { ...fresh.config.userProfiles, [saved.id]: userProfile } } }); setConfig(p.config); setUserDraft(null); await load(); if (!editing) await provisionUser(saved); } catch (err) { setError(err.message); } finally { setBusy(false); } };
