...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') return res.status(200).json({ keys: (await readPlatformKeys()).map(publicPlatformKey) });
    if (req.method === 'POST') {
      const config = await readPbxConfig();
      const created = await createPlatformKey({ ...req.body, organizationId: req.body?.organizationId || config.activeOrganizationId });
