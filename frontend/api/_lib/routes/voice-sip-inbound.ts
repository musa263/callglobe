...
import { lookupSipInbound, lookupSipInboundDigit } from '../sip-inbound.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const lookup = req.body?.digit
      ? await lookupSipInboundDigit(to, String(req.body.digit), config)
      : await lookupSipInbound(to, config);
