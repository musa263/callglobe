...
const dependencies = { readExtensionDirectory, readPbxConfig, readTenantSaasState, activeTenantAdmin, isExtensionSessionRevoked };
...
export function createSipRegistrationAccess(deps = dependencies) {
  return async (credential: StoredSipCredential): Promise<boolean> => {
    ...
    const extension = directory?.find((item) => item.id === credential.extensionId);
