...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') return res.status(200).json({ keys: (await readPlatformKeys()).filter((item) => item.organizationId === organizationId).map(publicPlatformKey) });
    if (req.method === 'POST') {
      const created = await createPlatformKey({ ...req.body, organizationId });
    ...
    if (!existing || existing.organizationId !== organizationId) return res.status(404).json({ error: 'API key not found.' });
