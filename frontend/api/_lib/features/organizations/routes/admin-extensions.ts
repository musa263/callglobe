import { readExtensionDirectory } from '../extension-store.js';
import { acquireTenantMutation } from '../tenant-mutation.js';
...
const dependencies = { acquireTenantMutation, readExtensionDirectory, requireAdmin, readPbxConfig, accessForSession, listExtensions, createExtension, updateExtension, deleteExtension,
...
export function createAdminExtensionsHandler(deps = dependencies) {
  const { acquireTenantMutation, readExtensionDirectory, requireAdmin, readPbxConfig, accessForSession, listExtensions, createExtension, updateExtension, deleteExtension,
  ...
  try {
    ...
    if (req.method !== 'GET') release = await acquireTenantMutation(organizationId);
    ...
    if (req.method === 'POST') {
      ...
      if (subscriptionAccess.superadmin === false) {
        const used = (await readExtensionDirectory() || await listExtensions(organizationId)).filter((item) => item.organizationId === organizationId && item.status === 'active').length;
    ...
    if (req.method === 'PATCH') {
      ...
      if (extension && isCompanyAccountRole(extension.role) && (linkedAdmin || isCompanyAdministrator(extension.role) || req.body?.loginPassword)) {
        ...
        // Restore the directory/carrier identity if the linked login could not be saved.
        await updateExtension(id, existing, organizationId);
        throw error;
      }
