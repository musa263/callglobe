...
import { findExtension, listExtensionSipUsernames } from '../../organizations/pbx.js';
...
import { extensionSipUri } from '../../calling/internal-sip.js';
import { callAction, dialCall, primaryVoiceCallerId } from '../../calling/voice-control.js';
import { normalizeE164 } from '../../organizations/tenancy.js';
import { sendIncomingCallWebPush } from '../../push/web-push-dispatcher.js';
import { userAvailableBySchedule } from '../../organizations/office-hours.js';
import { userNoAnswerSeconds, userVoicemailEnabled } from '../../organizations/user-call-routing.js';
...
const defaultDependencies = {
  verifyAiTransferToken, findExtension, readPbxConfig, readBusinessVoiceConfig,
  claimReplayKey, releaseReplayKey, listExtensionSipUsernames, primaryVoiceCallerId,
  callAction, dialCall, sendIncomingCallWebPush,
};

export function createAiTransferHandler(overrides: Partial<typeof defaultDependencies> = {}) {
  const {
    verifyAiTransferToken, findExtension, readPbxConfig, readBusinessVoiceConfig,
    claimReplayKey, releaseReplayKey, listExtensionSipUsernames, primaryVoiceCallerId,
    callAction, dialCall, sendIncomingCallWebPush,
  ...
  return async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const requestedExtension = typeof req.body?.extension === 'string' ? req.body.extension.trim() : '';
    ...
    try {
      ...
      if (!pbx.ai.enabled || !pbx.ai.transferEnabled || pbx.ai.assistantId !== authorization.assistantId) {
        return res.status(403).json({ error: 'AI transfer permission has changed. Please contact the company directly.' });
      }
      if (extension.organizationId !== authorization.organizationId) {
        return res.status(404).json({ error: 'The company extension is not available.' });
      }
      ...
      // A request that lost the claim does not own the caller's playback.
      // After dialing starts the result can be uncertain; retain the claim so
      // retries cannot create another leg with a different destination.
      if (claimed && assistantStopped && !dialAttempted) {
        ...
        await callAction(authorization.callControlId, 'ai_assistant_start', {
          assistant: { id: authorization.assistantId },
          command_id: `${commandPrefix(authorization.callControlId)}-resume-ai`,
        }).catch((resumeError) => console.warn('Vocivo could not resume AI after transfer failure', publicError(resumeError)));
      }
      if (claimed && !dialAttempted) {
        await releaseReplayKey(transferKey).catch((releaseError) => console.warn('Vocivo could not release failed AI transfer claim', publicError(releaseError)));
      }
...
export default createAiTransferHandler();
